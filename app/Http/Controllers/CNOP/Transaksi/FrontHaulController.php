...
class FrontHaulController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $check = TrWo::where('dasar_order', $request->dasar_order)->first();
            // $check = TrWo::where('dasar_order', $request->dasar_order)->where('tipe_ba', 'FRONTHAUL')->first();
            ...
            if (!$check) {
                ...
                $wo = new TrWo();
                $wo->id = $wo_id;
                $wo->dasar_order = $request->dasar_order;
                $wo->tipe_ba = 'FRONTHAUL';
                $wo->save();
            } else {
                $wo_id = $check->id;
            ...
            // $check_site = TrWoSite::where('site_id', $request->site_id)->where('tipe_ba', 'FRONTHAUL')->first();
            // if (!$check_site) {
            ...
            //     $wo_site->data_3g = 0; 
            //     $wo_site->data_4g = 0; 
            //     $wo_site->jarak_m = $request->jarak_m;
            //     $wo_site->sow = $request->sow;
            //     $wo_site->dibuat_oleh = Auth::user()->id;
            //     $wo_site->status = 'OA';
            //     $wo_site->jumlah = 0;
            //     $wo_site->progress = false;
            //     $wo_site->tipe_ba = 'FRONTHAUL';
            //     $wo_site->tahun_order = $request->tahun_order; //date('Y');
            //     $wo_site->sow = $request->sow;
    ...
    public function createBA(Request $request)
    {
        ...
        try {
            ...
            foreach ($sites as $site) {
                TrWoSite::where('tipe_ba', 'FRONTHAUL')
                ->where('progress', true)
                ->where('status', 'OA')
                ->where('tsel_reg', $request->tsel_reg)
    ...
    public function deleteBA($id)
    {
        ...
        try {
            ...
            if ($check) {
                ...
                TrWoSite::where('ba_id', $id)->where('tipe_ba', 'FRONTHAUL')
                ->update(array(
                    'ba_id' => null,
                )); 
    
                TrBa::where('id', $id)->where('tipe', 'FRONTHAUL')->delete();
    ...
    public function fileBA($id)
    {
        ...
        foreach ($data_wo as $w) {
            ...
            foreach ($data_sites as $site) {
                ...
                $otdr = TrWoSiteImage::where('wo_id', $site->wo_id)
                                ->where('wo_site_id', $site->wo_site_id)
                                ->where('tipe', 'OTDR')
                                ->first();

                $site->otdr = $otdr->image_url;

    
                $total_site++;
                $total_jarak_m = $total_jarak_m + $site->jarak_m;
        ...
        $pdf = PDF::loadView('fronthaul', [
            'jenis_dokumen'     => $jenis_dokumen,
            'data_wo'           => $data_wo,
            'data_site'         => $data_site,
            'data_ba'           => $data_ba,
            'dasar_permintaan'  => $dasar_permintaan,
            'total_jarak_m'     => $total_jarak_m,
