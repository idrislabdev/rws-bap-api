...
class RelokasiController extends Controller
{
    ...
    public function updateAlproSite(Request $request, $wo_id, $wo_site_id)
    {
        $v = Validator::make($request->all(), [
            'alpro_site' => 'required',
        ]);
    ...
    public function createBA(Request $request)
    {
        ...
        try {
            ...
            foreach ($sites as $site) {
                TrWoSite::where('tipe_ba', 'RELOKASI')
                    ->where('progress', true)
                    ->where('status', 'OA')
                    ->where('tsel_reg', $request->tsel_reg)
    ...
    public function createBAByPass(Request $request)
    {
        ...
        try {
            ...
            $count = 0;
            foreach ($sites as $site) {
                $update = TrWoSite::where('tipe_ba', 'RELOKASI')
    ...
    public function deleteBA($id)
    {
        ...
        try {
            ...
            if ($check) {
                ...
                TrWoSite::where('ba_id', $id)->where('tipe_ba', 'RELOKASI')
                    ->update(array(
                        'ba_id' => null,
                    ));

                TrBa::where('id', $id)->where('tipe', 'RELOKASI')->delete();
