...
class BeritaAcaraController extends Controller
{
    ...
    {
        $data = TrOloBa::with('dibuatOleh')->where('tipe_ba', 'OLO_TIF');
        if (isset($_GET['page'])) {
    ...
    public function checkNomor(Request $request)
    {
        ...
        try {
            ...
            if ($request->jenis_ba === 'BAST') {
                $no_dokumen_bast = UtilityHelper::checkNomorDokumenTIF();
                $data = new MaNomorDokumen();
                $data->id = Uuid::uuid4()->toString();
                $data->no_dokumen = $no_dokumen_bast;
                $data->tipe_dokumen = 'OLO_TIF_BAST';
                ...
                $no_dokumen_baut = UtilityHelper::checkNomorDokumenTIF();
                $data = new MaNomorDokumen();
                $data->id = Uuid::uuid4()->toString();
                $data->no_dokumen = $no_dokumen_baut;
                $data->tipe_dokumen = 'OLO_TIF_BAUT';
                $data->tgl_dokumen = date('Y-m-d');
                $data->save();
